...
fn main() {
    ...
    let code = match dispatch(&command, &globals) {
        Ok(code) => code,
        Err(error) => match cli::exit::report(error) {
            Some(code) => code,
            None => cellgov_terminal::interrupt::exit_interrupted(),
        },
    };
    std::process::exit(i32::from(code.value()));
}

fn dispatch(command: &Command, globals: &Globals) -> Result<CommandExitCode, CommandError> {
    ...
    match command {
        ...
        Command::Title(TitleCommand::Verify { title_id, ver }) => read::title_verify(
        ...
        Command::Scenario(ScenarioCommand::Dump { name }) => {
...
fn dispatch_dev(
    ...
    match dev {
        ...
        DevCommand::Funcs(args) => {
            funcs::run(args, vfs_flag)?;
            Ok(CommandExitCode::SUCCESS)
        }
        DevCommand::Lv2Discover(args) => {
            lv2_discover::run(args, vfs_flag, globals.format)?;
            Ok(CommandExitCode::SUCCESS)
        }
        DevCommand::Lv2Census(args) => {
            lv2_census::run(args, vfs_flag)?;
            Ok(CommandExitCode::SUCCESS)
        }
        #[cfg(feature = "decrypt")]
        ...
        DevCommand::Rpcs3Attribute(args) => {
            cli::rpcs3_attribute::run(args)?;
            Ok(CommandExitCode::SUCCESS)
        }
        DevCommand::FixtureGen(args) => cli::fixture_gen::run(args, vfs_flag),
        DevCommand::TitlesGen(args) => cli::titles_gen::run(args),
        DevCommand::CliGen(args) => {
            cli::cli_gen::run(args)?;
            Ok(CommandExitCode::SUCCESS)
        }
        DevCommand::WorkspaceGen(args) => {
...
/// # Errors
///
/// Returns an error if the command needs an empty VFS root override.
fn store_root(
    output: &parse::VfsOutput,
    vfs_flag: Option<&Path>,
) -> Result<std::path::PathBuf, CommandError> {
    Ok(match &output.output {
        Some(dir) => dir.clone(),
