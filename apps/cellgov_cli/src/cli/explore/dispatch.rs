...
pub(crate) fn run(
    ...
    match (&args.command, &args.scenario) {
        (Some(ExploreCommand::Title(title_args)), _) => title::run(title_args, format, vfs_flag),
        (
        ...
        (None, None) => Err(CommandError::failed(
            "explore: no scenario, micro-test or title named",
        )),
    }
}
