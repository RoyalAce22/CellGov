...
use crate::game::step_loop::ring::{PcRing, SyscallRing};
use crate::game::step_loop::timing::StepTiming;

pub(in crate::game) struct StepLoopCtx<'a> {
    pub(in crate::game) steps: &'a mut usize,
    pub(in crate::game) distinct_pcs:
        &'a mut std::collections::BTreeSet<(cellgov_core::AddressSpaceId, u64)>,
    ...
    pub(in crate::game) pc_ring: PcRing,
    pub(in crate::game) last_tty: Option<TtyCapture>,
    pub(in crate::game) last_exit: Option<ProcessExitInfo>,
    pub(in crate::game) syscall_ring: SyscallRing,
    /// Top entries identify busy-loop bodies on max-steps.
    pub(in crate::game) pc_hits:
        &'a mut std::collections::BTreeMap<(cellgov_core::AddressSpaceId, u64), u64>,
