...
use crate::game::prx::{install_kernel_context_opd, pre_init_tls};
...
pub(in crate::game) fn prepare(mut opts: PrepareOptions<'_>) -> PreparedBoot {
    ...
    pre_init_tls(&elf_data, &mut image.mem);

    // 3. The guest heap floor, above everything now resident.
    let placement = firmware::place_guest_heap(&elf_data, &prx_modules, image.code_floor);
    ...
    if opts.print_banner {
        host::print_boot_banner(&opts, &image, &params, &prx_modules);
    }
    ...
    let shadow = entry::build_instruction_shadow(&rt, &placement, image.code_floor);
    ...
    finish::dump_boot_memory(&rt, opts.dump_mem_boot_addrs);
