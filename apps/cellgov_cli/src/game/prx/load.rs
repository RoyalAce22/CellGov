...
pub(in crate::game) fn load_firmware_prx(
    ...
    let tramp_base = page_align_up_u64(loaded.data_end);
    let stats = match patch_got_atomic(modules, mem, tramp_base, |nid| {
        loaded.exports.get(&nid).copied()
    }) {
        Ok(s) => s,
        Err(e) => {
    ...
    println!(
        "prx: loaded {} -- {} exports, {}/{} game imports resolved to firmware OPDs, \
         {} routed to unresolved-import trampoline",
        loaded.name,
        loaded.exports.len(),
        stats.resolved,
        stats.total,
    ...
    let data_end = stats.tramp_region_end.max(loaded.data_end);
    Some(PrxLoadInfo {
        name: loaded.name,
