...
//!
//! ```text
//! cargo test -p cellgov_cli --features firmware-corpus --test firmware_reloc_census --release \
...
fn regenerate_firmware_reloc_census() {
    ...
    writeln!(
        out,
        "Generated by `cargo test -p cellgov_cli --features firmware-corpus --test firmware_reloc_census --release -- --ignored regenerate_firmware_reloc_census`. The test writes this file directly via `std::fs::write`; no stdout redirection."
    )
...
/// The regenerator above is `#[ignore]` and built only with the
/// `firmware-corpus` feature (it needs a decrypted firmware corpus), so
/// this companion is what catches an applier type the local table
/// cannot label in CI.
#[test]
