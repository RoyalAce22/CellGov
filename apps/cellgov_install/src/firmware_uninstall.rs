...
use crate::store::record::{CoreOsRecord, InstallRecord};
...
pub struct FirmwareUninstallPlan {
    ...
    /// The record's `[core_os]` block, naming the kernel a verifying
    /// removal hashes with the modules; `None` for a record that
    /// predates the block.
