...
//! [`KeyPolicy`]. A build without the `decrypt` feature passes
//! plaintext images through and refuses every SCE wrapper with
...
pub fn to_plaintext_elf<'a>(
    ...
    open_sce_wrapper(bytes, policy).map(Cow::Owned)
}

#[cfg(feature = "decrypt")]
fn open_sce_wrapper(bytes: &[u8], policy: KeyPolicy<'_>) -> Result<Vec<u8>, SceError> {
    ...
    match policy {
        KeyPolicy::AppOnly => {
            ...
            match find_npd_header_info(bytes) {
                ...
                Ok(None) => decrypt_self_to_elf(bytes),
        ...
        KeyPolicy::Auto(resolver) => decrypt_self_to_elf_auto(bytes, resolver),
    }
}
