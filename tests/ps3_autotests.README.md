...
test suite -- a collection of small `.ppu.elf` programs and the TTY
...
The suite is licensed GPLv2 and is **not** vendored into this repo,
...
without the suite does not build it and CI stays green without
skipping anything:
...
Enabling the feature declares the suite present: a missing clone is a
...
Windows autocrlf. If your local clone of the upstream suite already
