#!/bin/bash
# Build the ppu_cond_prodcons microtest.
#
# Run inside the ps3dev Docker container:
#
#   docker run --rm -v /path/to/ppu_cond_prodcons:/src \
...
python3 "$COMMON/patch_toc.py" \
    "$OUT/ppu_cond_prodcons.elf" \
    "${PPU_PREFIX}-readelf" \
    "${PPU_PREFIX}-nm"

echo "=== Build complete ==="
ls -la "$OUT/ppu_cond_prodcons.elf"
