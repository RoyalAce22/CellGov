#!/bin/bash
# Build the mailbox_roundtrip microtest.
#
...
#
#   docker run --rm -v /path/to/mailbox_roundtrip:/src \
...
python3 "$COMMON/patch_toc.py" \
    "$OUT/mailbox_roundtrip.elf" \
    "${PPU_PREFIX}-readelf" \
    "${PPU_PREFIX}-nm"

echo "=== Build complete ==="
ls -la "$OUT/mailbox_roundtrip.elf" "$OUT/spu_main.elf"
