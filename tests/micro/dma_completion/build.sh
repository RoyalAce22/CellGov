#!/bin/bash
# Build the dma_completion microtest.
#
...
#
#   docker run --rm -v /path/to/dma_completion:/src \
...
python3 "$COMMON/patch_toc.py" \
    "$OUT/dma_completion.elf" \
    "${PPU_PREFIX}-readelf" \
    "${PPU_PREFIX}-nm"

echo "=== Build complete ==="
ls -la "$OUT/dma_completion.elf" "$OUT/spu_main.elf"
