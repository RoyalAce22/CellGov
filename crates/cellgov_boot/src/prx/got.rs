...
/// validation failure the function discards the staged batch and leaves
/// guest memory unchanged. Callers must advance their alloc floor past
/// [`GotPatchStats::tramp_region_end`].
pub(super) fn patch_got_atomic(
    modules: &[cellgov_ppu::prx::ImportedModule],
    mem: &mut GuestMemory,
    tramp_base: u64,
    lookup: impl FnMut(&str, u32) -> Option<u64>,
) -> Result<GotPatchStats, PrxLoadStageError> {
    let mut staging = StagingMemory::new();
    let result = stage_got_batch(modules, mem, tramp_base, lookup, &mut staging);
    // Every refusal below the batch boundary returns with the writes
    // still pending. `StagingMemory` refuses a drop that still holds
    // them, so one discard here covers every such return.
    if result.is_err() {
        staging.clear();
    }
    result
...
/// The staging half of [`patch_got_atomic`], which owns the buffer.
fn stage_got_batch(
    modules: &[cellgov_ppu::prx::ImportedModule],
    mem: &mut GuestMemory,
    tramp_base: u64,
    mut lookup: impl FnMut(&str, u32) -> Option<u64>,
    staging: &mut StagingMemory,
) -> Result<GotPatchStats, PrxLoadStageError> {
        ...
        for func in &module.functions {
            ...
            let opd_u32 = if let Some(addr) = lookup(&module.name, func.nid) {
                ...
                let slot_base = if let Some(&existing) = nid_to_tramp_opd.get(&func.nid) {
                    ...
                    stage_unresolved_trampoline(staging, slot_base, func.nid);
    ...
    // The batch holds one GOT write per import, plus an OPD and a body
    // per distinct trampoline. The import counts therefore undercount
    // what a refusal discards.
    let staged = staging.len();
    staging
        .drain_into(mem)
        .map_err(|source| PrxLoadStageError::GotBatchCommit { staged, source })?;
    Ok(stats)
}
