...
#[test]
fn a_refused_batch_counts_the_trampoline_writes_it_discards() {
    const NID: u32 = 0x1111_2222;
    let mut mem = GuestMemory::new(0x2000);
    // The GOT slot straddles the end of the address space, so the batch
    // fails validation after it stages the OPD and the body.
    let modules = vec![import_module("libA", &[(NID, 0x1FFF)])];
    let err = patch_got_atomic(&modules, &mut mem, 0x1000, |_, _| None)
        .expect_err("an unmapped GOT slot must refuse the batch");
    match err {
        PrxLoadStageError::GotBatchCommit { staged, .. } => assert_eq!(
            staged, 3,
            "one GOT slot plus the trampoline OPD and body were discarded"
        ),
        other => panic!("unexpected refusal {other}"),
    }
    assert_eq!(
        slot_word(&mem, 0x1000),
        0,
        "atomic batch left the trampoline region unchanged"
    );
}
