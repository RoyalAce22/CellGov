...
pub(crate) fn ascii_safe_preview(bytes: &[u8]) -> String {
    bytes
        .iter()
        .map(|&b| {
...
pub(crate) fn fetch_raw_at(mem: &GuestMemory, pc: u64) -> Option<u32> {
...
pub(crate) fn region_label_at(mem: &GuestMemory, addr: u64, len: u64) -> &'static str {
...
pub(crate) fn longest_readable_prefix(
...
pub(crate) fn format_hle_idx(idx: u32) -> String {
    format!("<hle-idx-{idx}>")
}
