...
/// here ends the process, and a refusal leaves the caller's options
/// untouched but the partly-built runtime dropped.
pub fn prepare(mut opts: PrepareOptions<'_>) -> Result<PreparedBoot, BootError> {
    let t_start = Instant::now();
    let elf_data = std::mem::take(&mut opts.elf_data);

    // 1. Guest address space, with the title image loaded into it.
    let mut image = image::load_image(&opts, &elf_data, t_start)?;
    ...
    let firmware::FirmwareSet {
        ...
        t_hle_bind,
        t_prx_load,
    } = firmware::load_firmware_set(&opts, &elf_data, &mut image.mem, image.code_floor, t_start)?;
    pre_init_tls(&elf_data, &mut image.mem, opts.sink.as_ref())?;

    // 3. The guest heap floor, above everything now resident.
    let placement = firmware::place_guest_heap(
        &elf_data,
        &prx_modules,
        image.code_floor,
        opts.sink.as_ref(),
    ...
    let params = params::resolve_boot_params(&opts, &elf_data)?;

    // 5. What the boot loaded.
    if opts.print_banner {
        host::report_boot_banner(&opts, &image, &params, &prx_modules);
    }
    ...
    let child_init = loaders::install_spawn_loader(&mut rt, &opts);

    // 9. Guest-visible content, mounts last.
    providers::register_sibling_images(&mut rt, opts.elf_path)?;
    providers::register_content(&mut rt, &opts)?;
    providers::register_mounts(&mut rt, &opts)?;

    // 10. The title's primary entry state.
    let mut state = image.state;
    entry::seed_primary_entry_state(&mut rt, &mut state, &opts, &params, image.entry)?;

    // 11. Predecode shadow over the entry state's executable code.
    let shadow =
        entry::build_instruction_shadow(&rt, &placement, image.code_floor, opts.sink.as_ref())?;
    ...
    let counts =
        module_start::run_module_starts(&mut rt, &prx_modules, &opts, primary_unit_id, kctx_opd)?;

    // 14. Debug patches and dumps.
    finish::apply_patch_bytes(&mut rt, &opts)?;
    finish::dump_boot_memory(&rt, opts.dump_mem_boot_addrs, opts.sink.as_ref());
