...
use super::{
    save_boot_observation, save_boot_summary_json, BootSummaryInputs, ObservationInputs,
...
fn process_exit_title() -> crate::manifest::TitleManifest {
    crate::manifest::TitleManifest::load_from_text(
        PROCESS_EXIT_TOML,
...
fn an_inconsistent_boot_summary_is_refused_before_any_file_is_created() {
    ...
    let err = save_boot_summary_json(BootSummaryInputs {
        path: out.to_str().unwrap(),
        title: &process_exit_title(),
        outcome: cellgov_compare::BootOutcome::RsxWriteCheckpoint,
        steps: 1,
        step_budget: cellgov_time::Budget::new(1),
