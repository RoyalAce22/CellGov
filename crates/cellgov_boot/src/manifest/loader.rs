...
/// Why a title manifest did not load.
#[derive(Debug, thiserror::Error)]
pub enum ManifestError {
    /// The manifest file did not read.
    ...
    /// The manifest is not the TOML the schema expects.
    ...
    /// The `[checkpoint]` table names a kind the runner has no trigger
    /// for.
    #[error(
    ...
    /// A `pc` checkpoint's address does not read as one.
    ...
    /// Two manifests claim one short name.
    #[error(
    ...
    DuplicateShortName {
        /// The short name both manifests claim.
        name: String,
        /// The manifest the registry read first.
        first: PathBuf,
        /// The manifest that collided with it.
        second: PathBuf,
        /// Whether the two files hold the same bytes.
        files_identical: bool,
    },
    /// Two manifests claim one content id.
    #[error(
    ...
    DuplicateContentId {
        /// The content id both manifests claim.
        content_id: String,
        /// The manifest the registry read first.
        first: PathBuf,
        /// The manifest that collided with it.
...
impl TitleManifest {
    /// Read and translate the manifest at `path`.
    ///
    /// # Errors
    ///
    /// The file does not read, or its TOML does not match the schema;
    /// see [`ManifestError`].
