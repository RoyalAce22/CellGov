//! Classifies a `sys_tty_write` buffer before the loop captures it.

use cellgov_mem::{ByteRange, GuestAddr, GuestMemory, MemError};

#[derive(Debug, PartialEq, Eq)]
pub(crate) enum TtyCaptureDecision {
...
pub(crate) fn classify_tty_capture(args: &[u64; 9], mem: &GuestMemory) -> TtyCaptureDecision {
    let buf = args[2];
    let len = args[3];
