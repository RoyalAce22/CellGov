...
fn write_tty_log(prefix: &[u8], payload: &[u8], suffix: &[u8]) -> (ScratchDir, PathBuf) {
    let dir = scratch_labeled("rpcs3_tty");
...
fn parse_tty_log_extracts_single_region() {
    ...
    let (_dir, path) = write_tty_log(b"", &payload, b"");
...
fn parse_tty_log_finds_tag_after_noise() {
    ...
    let (_dir, path) = write_tty_log(noise, &payload, b"\nDone.\n");
...
fn parse_tty_log_no_magic_returns_error() {
    let dir = scratch_labeled("rpcs3_tty_nomag");
    let path = dir.join("tty.log");
...
fn parse_tty_log_truncated_payload_returns_error() {
    let dir = scratch_labeled("rpcs3_tty_trunc");
    let path = dir.join("tty.log");
...
fn parse_tty_log_truncated_header_returns_error() {
    let dir = scratch_labeled("rpcs3_tty_hdr");
    let path = dir.join("tty.log");
...
fn a_second_frame_past_the_payload_is_refused_rather_than_resolved_by_position() {
    ...
    let (_dir, path) = write_tty_log(b"", &payload, &second);
...
fn an_offset_that_overflows_is_named_alongside_its_size() {
    let (_dir, path) = write_tty_log(b"", &[0u8; 8], b"");
