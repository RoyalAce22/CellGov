...
/// Execute a primary-63 op. Unimplemented FPSCR / estimate ops
/// retire as no-ops (`Continue` without a register write), matching
/// the not-modelled-FPSCR posture above.
pub fn execute_fp63(
    state: &mut PpuState,
    op: Fp63Op,
    frt: u8,
    ...
    let result_bits = match op {
        ...
        Fp63Op::Fmadd => a.mul_add(c, b).to_bits(),
        // [PPC-Book1 p:107 s:4.6.6] fmsub: FRT <- (FRA * FRC) - FRB.
        Fp63Op::Fmsub => a.mul_add(c, -b).to_bits(),
        ...
        Fp63Op::Fnmadd => negate_unless_nan(a.mul_add(c, b)).to_bits(),
        // [PPC-Book1 p:107 s:4.6.6] fnmsub: FRT <- -((FRA * FRC) - FRB).
        Fp63Op::Fnmsub => negate_unless_nan(a.mul_add(c, -b)).to_bits(),
        // [PPC-Book1 p:106 s:4.6.6] fmul: FRT <- FRA * FRC.
        Fp63Op::Fmul => (a * c).to_bits(),
        ...
        // [PPC-Book1 p:104 s:4.6.5] fcmpu / fcmpo: CR[BF] <- {LT,GT,EQ,FU}.
        ...
        // fctiw / fctiwz. A NaN input must produce 0x8000_0000;
        // Rust's `f64 as i32` returns 0 for NaN, so guard. ((+/-)inf
        // clamps via saturating cast already.) Result is
        // zero-extended into FRT[32:63]; FRT[0:31] is
        ...
        // fctid / fctidz. Same NaN guard at 64-bit width.
...
/// Execute a primary-59 op. `fsqrts` / `fres` retire as no-ops.
pub fn execute_fp59(
    state: &mut PpuState,
    op: Fp59Op,
    frt: u8,
