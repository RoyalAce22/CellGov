...
fn rlwnm_rotates_by_rb_low_5_bits() {
    let mut s = PpuState::new();
    s.set_gpr(0, 0x0000_0000_1234_5678);
    s.set_gpr(8, 8);
    exec_no_mem(
...
fn rlwnm_ignores_high_bits_of_rb() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(1, 0x0000_0000_DEAD_BEEF);
    s.set_gpr(2, 0x20);
    exec_no_mem(
        &PpuInstruction::Rlwnm {
            ra: 3,
...
fn rlwimi_preserves_ra_high_32() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(0, 0xCAFE_BABE_DEAD_BEEF);
    s.set_gpr(1, 0x0000_0000_0000_00FF);
    exec_no_mem(
...
fn srad_signed_64_bit_shift() {
    let mut s = PpuState::new();
    s.set_gpr(3, 0x8000_0000_0000_0000);
    s.set_gpr(4, 4);
    exec_no_mem(
        &PpuInstruction::Srad {
            ra: 5,
...
fn rldic_clears_both_sides() {
    ...
    // bits 0..=31 and 60..=63.
    let mut s = PpuState::new();
    s.set_gpr(4, 0xFFFF_FFFF_FFFF_FFFF);
    exec_no_mem(
...
fn rldimi_preserves_prior_ra_outside_mask() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(4, 0xDEAD_BEEF_CAFE_BABE); // RS
    s.set_gpr(5, 0x1111_2222_3333_4444); // prior RA
    exec_no_mem(
...
fn srad_shifts_full_64_bits_arithmetically() {
    let mut s = PpuState::new();
    s.set_gpr(4, 0xFFFF_FFFF_FFFF_FFF0); // -16
    s.set_gpr(5, 4);
    exec_no_mem(
        &PpuInstruction::Srad {
            ra: 3,
...
fn srad_dot_sets_cr0_and_preserves_ca() {
    let mut s = PpuState::new();
    s.set_gpr(3, (-1i64) as u64); // all-ones, guaranteed 1-bit shifted out.
    s.set_gpr(4, 1);
    exec_no_mem(
        &PpuInstruction::Srad {
            ra: 5,
...
fn rldimi_dot_sets_cr0_from_merged_value() {
    let mut s = PpuState::new();
    s.set_gpr(3, 0x1); // RS
    s.set_gpr(5, 0xFFFF_FFFF_FFFF_FFFF); // prior RA (bits outside mask preserved)
...
fn srawi_dot_sets_both_ca_and_cr0() {
    let mut s = PpuState::new();
    s.set_gpr(3, (-1i32) as u32 as u64);
    exec_no_mem(
...
fn sld_dot_sets_cr0_eq_when_shift_ge_64() {
    let mut s = PpuState::new();
    s.set_gpr(3, 0xFFFF_FFFF_FFFF_FFFF);
    s.set_gpr(4, 64); // shift >= 64 -> result 0
    exec_no_mem(
        &PpuInstruction::Sld {
            ra: 5,
            rs: 3,
...
fn srd_dot_sets_cr0_gt_when_high_bit_shifted_into_payload() {
    let mut s = PpuState::new();
    s.set_gpr(3, 0x8000_0000_0000_0000);
    s.set_gpr(4, 1); // logical shift right -> positive
    exec_no_mem(
        &PpuInstruction::Srd {
            ra: 5,
            rs: 3,
...
fn sraw_positive_rs_clears_ca() {
    let mut s = PpuState::new();
    s.set_gpr(3, 0x0000_0000_7FFF_FFFF); // positive
    s.set_gpr(4, 4);
    s.set_xer_ca(true);
    exec_no_mem(
        &PpuInstruction::Sraw {
...
fn sraw_negative_rs_with_one_bits_shifted_out_sets_ca() {
    let mut s = PpuState::new();
    s.set_gpr(3, 0xFFFF_FFFF); // all ones
    s.set_gpr(4, 4);
    s.set_xer_ca(false);
    exec_no_mem(
        &PpuInstruction::Sraw {
...
fn srad_positive_rs_clears_ca() {
    let mut s = PpuState::new();
    s.set_gpr(3, 0x7FFF_FFFF_FFFF_FFFF); // positive
    s.set_gpr(4, 4);
    s.set_xer_ca(true);
    exec_no_mem(
        &PpuInstruction::Srad {
...
fn srad_negative_rs_with_no_one_bits_shifted_out_clears_ca() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(3, 0xFFFF_FFFF_FFFF_FFF0);
    s.set_gpr(4, 4);
    s.set_xer_ca(true);
    exec_no_mem(
        &PpuInstruction::Srad {
...
fn sradi_positive_rs_clears_ca() {
    let mut s = PpuState::new();
    s.set_gpr(3, 0x7FFF_FFFF_FFFF_FFFF);
    s.set_xer_ca(true);
    exec_no_mem(
        &PpuInstruction::Sradi {
            ra: 5,
...
fn srawi_positive_rs_clears_ca() {
    let mut s = PpuState::new();
    s.set_gpr(3, 0x0000_0000_7FFF_FFFF);
    s.set_xer_ca(true);
    exec_no_mem(
        &PpuInstruction::Srawi {
            ra: 5,
...
fn srawi_negative_rs_with_no_one_bits_shifted_out_clears_ca() {
    let mut s = PpuState::new();
    s.set_gpr(3, 0xFFFF_FFF0); // negative i32, low 4 bits zero
    s.set_xer_ca(true);
    exec_no_mem(
        &PpuInstruction::Srawi {
            ra: 5,
...
fn rldcr_dot_sets_cr0_gt_on_positive() {
    let mut s = PpuState::new();
    s.set_gpr(3, 0x1);
    s.set_gpr(4, 8); // shift left by 8 -> 0x100
    exec_no_mem(
