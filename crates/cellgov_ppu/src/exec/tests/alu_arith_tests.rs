...
fn addze_oe_signed_overflow_sets_ov_and_so() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(4, 0x7FFF_FFFF_FFFF_FFFF);
    s.set_xer_ca(true);
    exec_no_mem(
        &PpuInstruction::Addze {
            rt: 3,
...
fn subfo_signed_overflow_sets_ov_and_so() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(3, (-1i64) as u64);
    s.set_gpr(4, i64::MAX as u64);
    exec_no_mem(
        &PpuInstruction::Subf {
            rt: 5,
            ra: 3,
...
fn subfe_clears_ca_on_borrow() {
    let mut s = PpuState::new();
    s.set_gpr(3, 10); // RA
    s.set_gpr(4, 3); // RB; rb - ra borrows
    s.set_xer_ca(true);
    exec_no_mem(
        &PpuInstruction::Subfe {
            rt: 5,
