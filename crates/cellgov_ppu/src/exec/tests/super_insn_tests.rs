...
fn clrldi_zero_mask_clears_all() {
    let mut s = PpuState::new();
    s.set_gpr(4, 0xDEAD_BEEF_CAFE_BABE);
    exec_no_mem(
...
fn srdi_large_shift() {
    let mut s = PpuState::new();
    s.set_gpr(4, 0x8000_0000_0000_0000);
    exec_no_mem(
        &PpuInstruction::Srdi {
            ra: 3,
            rs: 4,
...
fn mflr_std_writes_lr_then_stores_64_bits() {
    let mut s = PpuState::new();
    s.set_lr(0xDEAD_BEEF_CAFE_BABE);
    s.set_gpr(1, 0x1000);
    let mut effects = Vec::new();
    exec_with_mem(
...
fn li_loads_sign_extended_immediate() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(5, 0xDEAD_BEEF_CAFE_BABE); // pre-existing value clobbered
...
fn mr_same_register_is_identity() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(7, 0x1234_5678_9ABC_DEF0);
...
fn nop_advances_nia_no_state_change() {
    ...
    let mut s = PpuState::new();
    for i in 0..32 {
        s.set_gpr(i, i as u64 * 0x0101_0101_0101_0101);
    }
    s.set_lr(0xAAAA_BBBB_CCCC_DDDD);
    s.set_cr_field(0, 0b0100);
    s.set_xer_ov(true);
    let snapshot_gpr = *s.gpr.as_array();
    let snapshot_lr = s.lr();
    let snapshot_cr0 = s.cr_field(0);
    let snapshot_so = s.xer_so();
    let v = exec_no_mem(&PpuInstruction::Nop, &mut s);
    assert_eq!(v, ExecuteVerdict::Continue);
    assert_eq!(*s.gpr.as_array(), snapshot_gpr);
...
fn slwi_n1_shifts_low_32_then_zero_extends() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(4, 0xFFFF_FFFF_8000_0001);
...
fn srwi_n31_isolates_top_bit_of_low_word() {
    let mut s = PpuState::new();
    s.set_gpr(4, 0xFFFF_FFFF_8000_0000);
    exec_no_mem(
        &PpuInstruction::Srwi {
            ra: 3,
            rs: 4,
...
fn clrlwi_n0_zero_extends_low_32() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(4, 0xFFFF_FFFF_DEAD_BEEF);
...
fn sldi_n1_doubles_full_64_bits() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(4, 0x4000_0000_0000_0001);
...
fn srdi_n1_halves_full_64_bits_unsigned() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(4, 0x8000_0000_0000_0002);
...
fn stdstd_writes_both_doublewords_at_consecutive_eas() {
    ...
    s.set_gpr(6, 0x99AA_BBCC_DDEE_FF00);
    let mut effects = Vec::new();
    let v = exec_with_mem(
...
fn stdstd_negative_offset1_still_uses_ea2_eq_ea1_plus_8() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(1, 0x1100);
    s.set_gpr(5, 0xAAAA_AAAA_AAAA_AAAA);
    s.set_gpr(6, 0xBBBB_BBBB_BBBB_BBBB);
    let mut effects = Vec::new();
    let v = exec_with_mem(
        &PpuInstruction::StdStd {
            rs1: 5,
...
fn mflr_stw_does_not_touch_cr_or_xer() {
    ...
    let mut s = PpuState::new();
    s.set_lr(0x1234_5678_9ABC_DEF0);
    s.set_gpr(1, 0x1000);
...
fn stdstd_no_partial_commit_on_buffer_full() {
    ...
    let mut s = PpuState::new();
    s.set_gpr(1, 0x1000);
    s.set_gpr(5, 0x1111_1111_1111_1111);
    s.set_gpr(6, 0x2222_2222_2222_2222);
