...
```
Usage: cellgov dev fuzz replay [OPTIONS] --artifact <PATH>
```
...
```
Usage: cellgov dev fuzz evaluate [OPTIONS] --output <PATH> <ENGINE>
```

| Argument | Description |
| --- | --- |
| `ENGINE` | Engine every trial runs. One of `ppu-instruction`, `ppu-sequence`, `spu-instruction`, `spu-sequence`. Required. |

| Option | Value | Description |
| --- | --- | --- |
| `--trials` | `TRIALS` | Number of trials; each runs one consecutive seed from --first-seed. Default `10`. |
| `--first-seed` | `FIRST_SEED` | Seed of the first trial. Default `1`. |
| `--cases` | `CASES` | Case indices every trial considers. Default `100`. |
| `--sequence-words` | `SEQUENCE_WORDS` | Number of words generated for each sequence case. |
| `--strategy` | `STRATEGY` | Generate typed instruction forms or raw decoder words. One of `structured`, `raw-words`. Default `structured`. |
| `--finding-limit` | `FINDING_LIMIT` | Maximum detailed findings each trial retains. Default `20`. |
| `--reduction` | `REDUCTION` | Reduce each trial's retained findings and record the cost. One of `none`, `on-finding`. Default `none`. |
| `--reduction-policy` | `REDUCTION_POLICY` | Candidate selection policy for reduction. One of `deterministic`, `greedy`. Default `deterministic`. |
| `--reduction-budget` | `REDUCTION_BUDGET` | Maximum candidate evaluations spent on each finding. Default `4096`. |
| `--workers` | `WORKERS` | Trials run at once; defaults to available parallelism. |
| `--progress` | -- | Report a line after each trial. |
| `--output` | `PATH` | Write the versioned JSON results here. Required. |
| `--baseline` | `PATH` | Stored evaluation to rank these results against. |
...
##### `cellgov dev fuzz compare`

Rank a stored evaluation against a baseline at equal budgets.

```console
$ cellgov dev fuzz compare --baseline baseline.json --candidate candidate.json
```

```
Usage: cellgov dev fuzz compare [OPTIONS] --baseline <PATH> --candidate <PATH>
```

| Option | Value | Description |
| --- | --- | --- |
| `--baseline` | `PATH` | Stored evaluation to rank the candidate against. Required. |
| `--candidate` | `PATH` | Stored evaluation under review. Required. |
...
```
Usage: cellgov dev fuzz smoke [OPTIONS]
```

| Option | Value | Description |
| --- | --- | --- |
| `--artifacts-dir` | `DIR` | Directory that receives one minimized artifact per retained finding. Default `target/fuzz-smoke`. |
| `--regressions` | `DIR` | Regression directory whose open entries cover known findings. |
| `--reduction-budget` | `REDUCTION_BUDGET` | Maximum candidate evaluations spent reducing each finding. Default `4096`. |
| `--progress` | -- | Report a line after each campaign. |
...
```
Usage: cellgov dev fuzz promote [OPTIONS] --artifact <PATH> --regressions <DIR> --name <NAME> --summary <SUMMARY>
```

| Option | Value | Description |
| --- | --- | --- |
| `--artifact` | `PATH` | Minimized finding artifact to promote. Required. |
| `--regressions` | `DIR` | Regression directory that receives the artifact and the manifest entry. Required. |
| `--name` | `NAME` | Name of the regression: lowercase letters, digits, '-' and '_'. Required. |
| `--summary` | `SUMMARY` | What the finding is, in one sentence. Required. |
| `--profile` | `PROFILE` | Build profile the finding reproduces in. One of `both`, `debug`, `release`. Default `both`. |
