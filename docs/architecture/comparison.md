...
events-only, prefix. Multi-baseline mode checks oracle agreement
across, e.g., the RPCS3 interpreter and LLVM before declaring a
CellGov divergence.
...
flowchart LR
  ...
  oc --> cmp["compare-observations: strict / memory / events / prefix"]
  orr --> cmp
