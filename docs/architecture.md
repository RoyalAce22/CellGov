...
| `cellgov_trace`                | Binary trace format: 13 record variants with strict tag/layout contract (7 decision-level + `PpuStateHash` + `PpuStateFull` for per-step divergence trace + `HostInvariantBreak` side-channel + `SyscallEntered` / `SyscallReturned` syscall entry and return records + `ReservedRegionRead` locating each provisional zero-read by step and address).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
current PC. Batching it over precomputed basic-block lengths was
measured and rejected: the address resolution a batch removes is
worth under 1 ns per instruction, while the loop-carried window
state a batch must carry costs about 4 ns.
...
- **13 trace record variants** in `cellgov_trace::TraceRecord` --
  ...
  (`HostInvariantBreak`), a syscall-entry record
  (`SyscallEntered`) emitted before `Lv2Host::dispatch` runs and
  its return counterpart (`SyscallReturned`: caller, `r3` value,
  guest time) emitted when the value is stored for the caller --
  in the same commit for an immediate return, at wake time for a
  call that blocked -- so a wrong errno is readable from the
  stream without a re-run, and
...
truth that the firmware reloc census also consults), and the
...
| `sys_spu_initialize`                               | 169                     | CELL_EINVAL when `max_raw_spu > 5`, otherwise CELL_OK: the deterministic oracle does not partition the SPU pool into "usable" vs "raw" slots. The announced limits are not persisted -- they are reported as a `dispatch.spu_initialize_limits_unpersisted` invariant break and discarded.                                                                                                                                                                                       |
...
| `sys_tty_write`                                    | 403                     | Appends the buffer to `tty_log` and writes `len` back through `nwritten_ptr`; CELL_EFAULT when that pointer is null. fd / len / buf carried in `Lv2Request` for tracing.                                                                                                                                                                                                                                                                                                        |
| `sys_fs_open`                                      | 801                     | Routes through the in-memory FS layer (see "In-memory filesystem" below). Registered paths get a fresh fd from `FsStore`; a miss falls through to the mount table before returning `CELL_FS_ENOENT`. `/app_home/PARAM.SFO` and `/app_home/output.txt` are not a synthetic-fd whitelist -- they are pre-registered as empty `FsStore` blobs at host construction and take the ordinary path. CELL_EROFS on a write-flagged open of an existing path (read-only FS model); CELL_EFAULT on unmapped path / fd out-pointer; CELL_EINVAL on no-NUL within `CELL_FS_MAX_PATH_LENGTH`; CELL_EMFILE when the FsStore allocator is exhausted. |
...
| `sys_fs_close`                                     | 804                     | FsStore-tracked fds are removed from the open-fd table so subsequent reads / fstats return EBADF. Unknown fds return CELL_EBADF. `fs_fd_count` is left unchanged across close (real-PS3 invariant pinned by ps3autotests `sys_process`).                                                                                                                                                                                                                                        |
...
| `sys_fs_opendir`                                   | 805                     | Snapshots the mount-resolved directory entries into a per-fd `BTreeMap<u32, DirEntry>`, sorted lexicographically by byte order (the mount table is the only source; there is no manifest-directory path). Returns a fresh dir-fd via `FsStore::open_dir`. CELL_FS_ENOENT for unknown paths, plus ENOTDIR / EACCES / EIO / EMFILE arms.                                                                                                                                          |
...
enumeration -- it is rendered from `cellgov_lv2::request::fidelity`
and a dispatch probe fails CI when the two disagree. This table is
hand-maintained narrative for the arms worth describing in prose;
consult the generated doc for coverage questions. Typed arms
deliberately omitted here as self-describing id-mint or
constant-return stubs: `sys_process_getpid` (1),
...
672, 674, and 675 (677 also exists as a routed stub). The surface is a single allocated RSX
...
| ------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 668     | `SysRsxMemoryAllocate`   | Bump-allocates the caller's requested `size` (not a fixed amount) from `SYS_RSX_MEM_BASE = 0x3000_0000`, mints a monotonic handle, and writes two out-params: `mem_handle` (u32) and `mem_addr` (u64). CELL_ENOMEM on `size == 0`, wrap, or past `SYS_RSX_MEM_END`. The 3 MB figure is `region::CONTEXT_RESERVATION`, which belongs to 670.                                            |
...
| 674     | `SysRsxContextAttribute` | Sub-command dispatch: FIFO_SETUP, FLIP_MODE, FLIP_BUFFER, SET_DISPLAY_BUFFER, and three CellGov-internal handler-register packages (SET_FLIP_HANDLER 0x8000_0108, SET_VBLANK_HANDLER 0x8000_010C, SET_USER_HANDLER 0x8000_010D). FIFO_SETUP records initial get / put pointers on the context and writes them through to the MMIO control register when the target words are writable (which the RSX mirror is what normally makes true). |
...
| 677     | (routed `Unsupported`)   | `sys_rsx_attribute`: CELL_OK with no state change, plus an invariant break. Listed here for completeness -- it dispatches through the routed-`Unsupported` table above, not a typed sys_rsx request.                                                                                                                                                                                |
...
trampoline. Those ELFs do import firmware-side NIDs -- a dozen
`sysPrxForUser` entries apiece -- so suppressing the firmware set
is what forces them down the trampoline path, not an absence of
imports to resolve.
...
`CELL_ENOENT` (witnessed as `prx_load_not_found`), and its atexit
sequence walks every registered module id through the
`_sys_prx_stop_module` handshake (the routed 482 arm) before its
`_sys_prx_unload_module` calls land in the routed 483 arm.
